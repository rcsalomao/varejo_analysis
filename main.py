...
def get_dataframes(filename):
    # Ingestão dos dados e criação do relatório de entrada
...
def process_limpeza_df(df: pd.DataFrame):
    print(">>> Limpeza do dataframe")
...
def explor_agrup(df):
    ...
    # as clientes do gênero feminino são mais predominantes.
    # Entretanto há uma inversão significativa para o caso específico de clientes
    # com 4 filhos, cujo há uma presença mais exarcebada de pessoas do gênero masculino.
    print()
...
def insights(df):
    ...
    print("  Estatísticas das vendas por mês:")
    ...
    print(
        df.query("PR_CAT == 'ALIMENTOS'")["PR_NOME"].value_counts(normalize=True) * 100
    )
    print()
